...
function countMarkedNumbers(player) {
    ...
    player.cards.forEach(card => {
        // Usa filter invece di forEach con if per prestazioni migliori
        total += card.numbers.filter(num => num.marked).length;
    });
    return total;
}
...
// Controlla se un giocatore ha cliccato sui numeri necessari per una vincita (ottimizzato)
function checkPlayerMarkedNumbers(player, roomCode, winType, cardIndex, rowIndex) {
    const card = player.cards[cardIndex];
    if (!card || !card.rows[rowIndex]) return false;
    
    const row = card.rows[rowIndex];
    const extractedNumbers = rooms.get(roomCode)?.extractedNumbers || [];
    
    // Controlla solo i numeri non nulli nella riga
    const clickedCount = row.filter(cell =>
        cell &&
        extractedNumbers.includes(cell.number) &&
        cell.marked
    ).length;
    
    // Verifica se ha cliccato abbastanza numeri per la vincita
    ...
    return clickedCount >= (requiredNumbers[winType] || 0);
}

// Controlla tutte le possibili vincite per un giocatore (ottimizzato)
function checkAllPossibleWins(player, roomCode) {
    ...
    player.cards.forEach((card, cardIndex) => {
        ...
        card.rows.forEach((row, rowIndex) => {
            // Calcola una volta sola i numeri estratti
            const extractedNumbers = room?.extractedNumbers || [];
            
            // Controlla solo se il giocatore ha cliccato i numeri necessari
            const markedInRow = row.filter(cell =>
                cell &&
                extractedNumbers.includes(cell.number) &&
                cell.marked
            ).length;
            
            // Controlla tutti i tipi di vincita in un solo ciclo
            const winTypes = [
                { type: 'ambo', required: 2 },
                { type: 'terna', required: 3 },
                { type: 'quaterna', required: 4 },
                { type: 'cinquina', required: 5 }
            ];
            
            winTypes.forEach(({ type, required }) => {
                if (markedInRow >= required && !hasWinTypeBeenMade(roomCode, type)) {
                    const winData = {
                        playerName: player.name,
                        playerId: player.id,
                        type: type,
...
// Controlla se ci sono vincite multiple dello stesso tipo in una stanza
function checkForSharedWins(roomCode, winType) {
    const history = winHistory.get(roomCode) || [];
    const winsOfType = history.filter(win => win.type === winType);
    
    if (winsOfType.length > 1) {
        // Più vincite dello stesso tipo, trova i giocatori
        const playerNames = winsOfType.map(win => win.playerName);
        return {
            type: winType,
            players: playerNames,
            timestamp: new Date().toISOString()
...
io.on('connection', (socket) => {
    ...
    socket.on('extract-number', (roomCode) => {
        ...
        // Segna il numero su tutte le cartelle dei giocatori (ottimizzato)
        room.players.forEach(player => {
            player.cards.forEach(card => {
                // Cerca solo il numero specifico invece di iterare su tutti
                const numObj = card.numbers.find(n => n.number === newNumber);
                if (numObj) {
                    numObj.marked = true;
                }
            });
            // Calcola markedCount solo se necessario
            player.markedCount = countMarkedNumbers(player);
        });
        
        // Invia a tutti nella stanza SOLO il numero corrente
        // Le vincite vengono rilevate solo quando i giocatori cliccano sui numeri
    ...
    // Giocatore segnala una vincita (dopo aver cliccato i numeri)
    socket.on('player-win', (data) => {
        const { roomCode, playerName, cardIndex, rowIndex, type } = data;
        const room = rooms.get(roomCode);
        
        if (!room) {
            console.error('Stanza non trovata per la vincita:', roomCode);
            return;
        }
        
        // Crea dati della vincita
        const winData = {
        ...
        // Registra la vincita (solo se è la prima di quel tipo)
        if (recordWin(roomCode, winData)) {
            // Controlla se ci sono vincite condivise
            const sharedWin = checkForSharedWins(roomCode, type);
            
            if (sharedWin) {
                // Notifica vincita condivisa
                const sharedMessage = `🎉 ${sharedWin.players.join(' e ')} hanno fatto ${type.toUpperCase()} insieme! 🎉`;
                io.to(roomCode).emit('shared-win', {
                    type: 'shared',
                    winType: sharedWin.type,
                    players: sharedWin.players,
                    message: sharedMessage
                });
                io.to(`admin-${roomCode}`).emit('shared-win', {
                    type: 'shared',
                    winType: sharedWin.type,
                    players: sharedWin.players,
                    message: sharedMessage
                ...
                // Notifica vincita singola
                if (type === 'tombola') {
                    io.to(roomCode).emit('player-won', winData);
                    io.to(`admin-${roomCode}`).emit('player-won', winData);
                } else {
                    io.to(roomCode).emit('win-detected', winData);
                    io.to(`admin-${roomCode}`).emit('win-detected', winData);
                }
            }
            
            // Se c'è stata una tombola, blocca ulteriori estrazioni
            if (type === 'tombola') {
                // Ferma l'estrazione automatica se attiva
                clearInterval(autoExtractInterval);
                autoExtractInterval = null;
                
                // Notifica che il gioco è finito
                io.to(roomCode).emit('game-finished', {
                    winner: playerName,
                    message: `🎉 ${playerName} ha fatto TOMBOLA! Il gioco è terminato! 🎉`
                });
                io.to(`admin-${roomCode}`).emit('game-finished', {
                    winner: playerName,
                    message: `🎉 ${playerName} ha fatto TOMBOLA! Il gioco è terminato! 🎉`
                });
            }
            
            console.log(`Vincita ${type} segnalata da ${playerName} nella stanza ${roomCode}`);
        } else {
            console.log(`Vincita ${type} già fatta nella stanza ${roomCode}`);
        }
    });
