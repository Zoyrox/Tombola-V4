...
io.on('connection', (socket) => {
    ...
    socket.on('extract-number', (roomCode) => {
        ...
        room.players.forEach(player => {
            ...
            playerWins.forEach(win => {
                allWins.push(win);
                winningPlayers.add(win.playerId);
            });
        });
        ...
        allWins.forEach(win => {
            ...
            const sharedWin = checkForSharedWins(roomCode, win.type);
            
            if (sharedWin) {
                // Notifica vincita condivisa
                const sharedMessage = `🎉 ${sharedWin.players.join(' e ')} hanno fatto ${win.type.toUpperCase()} insieme! 🎉`;
        ...
        const tombolaWin = allWins.find(win => win.type === 'tombola');
        if (tombolaWin) {
            ...
            io.to(roomCode).emit('game-finished', {
                winner: tombolaWin.playerName,
                message: `🎉 ${tombolaWin.playerName} ha fatto TOMBOLA! Il gioco è terminato! 🎉`
            });
    ...
    socket.on('mark-number-manual', (data) => {
        ...
        if (found) {
            ...
            // Se c'è stata una tombola, blocca ulteriori estrazioni
            const tombolaWin = newWins.find(win => win.type === 'tombola');
            if (tombolaWin) {
                // Ferma l'estrazione automatica se attiva
                clearInterval(autoExtractInterval);
                autoExtractInterval = null;
